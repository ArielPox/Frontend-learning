...
#给列起一个别名：oldname as newname 其中的as可以省略 newname可以使用一个双引号括起来
...
--  空值null参与运算还是null 这里的null 不是0 不是“” 不是“null” 表示的就是一个未知的数值
...
-- 着重号``：当起名与关键字冲突的时候就可以将起名用``保护起来
SELECT *
FROM `order`;

-- 查询常数 就是在select的后面加上一个常数 这个常数会被添加到所有的字段里面
SELECT '查询常数',123,employee_id,last_name
FROM employees;

-- 显示表结构 显示的是表中的字段的详细的信息
DESCRIBE employees;
